            ...
            for k, v in pairs(receptionPeds) do
                ...
                if DoesEntityExist(v) then
                    if GetDistanceBetweenCoords(GetEntityCoords(me), GetOffsetFromEntityInWorldCoords(v, 0.0, 2.0, 0.0), true) <= 1.5 then 
                        ...
                        lib.showTextUI((Strings['get_help']):format('[E]', Config.Price), {
                        ...
                        if IsControlJustReleased(0, 38) or IsDisabledControlJustReleased(0, 38) then
                            TriggerServerEvent('esx_hospitalbed:gethelp', k)
                        end
                    end
                end
            end
...
RegisterNetEvent('esx_hospitalbed:revive')
AddEventHandler('esx_hospitalbed:revive', function(hospital)
    Cinema = true
...
RegisterNetEvent('esx_hospitalbed:notify')
AddEventHandler('esx_hospitalbed:notify', function(txt)
